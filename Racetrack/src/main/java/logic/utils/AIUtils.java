	...
	public static List<Point2D> CalculateAccelerations(Point2D startPosition, Point2D endPosition, Point2D startSpeed, Point2D endSpeed, int turns)
	{
		...
		if (((tx > ty) || overshootY) && !overshootX)
		{
			...
			if (Math.signum(sy2) * (Dy-dminY) >= 0)
			{
				...
				if (turns != tx)
				{
					System.out.println(String.format("Mismatch between turns = %d and tx = %d", turns, tx));
				}
			}
			else
			{
				...
				if (turns != t_new)
				{
					System.out.println(String.format("Mismatch between turns = %d and tx_new = %d", turns, t_new));
				}
			}
		}
		else

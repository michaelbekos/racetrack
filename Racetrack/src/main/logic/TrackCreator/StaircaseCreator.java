...
public class StaircaseCreator
{
	...
	private static void writeStaircaseXML(int width, int numberOfTurns, int lengthOfStraightSegments)
	{
		...
		while (!countOkay)
		{
			...
			{
				file = new File(System.getProperty("user.dir") + String.format("/Tracks/Staircase-%d.xml",randomTrackCount));
			}
			else
			{
				file = new File(System.getProperty("user.dir") + String.format("\\Tracks\\Staircase-%d.xml",randomTrackCount));
		...
		trackElement.setAttribute("name", String.format("Staircase-%d",randomTrackCount));
		...
		try
		{
			if( System.getProperty("os.name").equalsIgnoreCase( "Linux" ) )
			{
				out.output( trackFile, new FileOutputStream(System.getProperty("user.dir") + String.format("/Tracks/Staircase-%d.xml",randomTrackCount)));
			}
			else
			{
				out.output( trackFile, new FileOutputStream(System.getProperty("user.dir") + String.format("\\Tracks\\Staircase-%d.xml",randomTrackCount)));
